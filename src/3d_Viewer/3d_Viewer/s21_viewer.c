...
/*
This function processes the incoming file and populates the structures with the necessary information.
There is two readings of file: 1) to count number of vertexes and polygons, 2) to fill them
*/
int parse_file(const char *file_name, obj_data *total_data) {
    ...
    if (is_ok) {
        ...
        if (is_ok) {
            ...
            total_data->polygons = (polygon *) calloc(total_data->number_polygons, sizeof(polygon));
...
/*
There is function for initialization matrix of coordinate. There is memory allocation for matrix.
CoordMatrix have empty 0 row, because at the polygons count starts from 1
*/
int init_coord_matrix(coord_matrix *coordMatrix, unsigned int number_vertexes,
    ...
    coordMatrix->coordinates = (double **) calloc(coordMatrix->rows, sizeof(double *));
...
// This function check that token is double number. First letter can be minus, token can consist only one dot, and can not contain any of letter
...
// This function checks that there is no index greater than the number of vertexes.
...
// There is support function to free matrix with information about coordinates of all vertexes
...
// There is support function to free matrix with information about vertex's indexes
...
// There is support function to transformation matrix to field [-1;1] by each coordinates
...
/*
There is support function to move object to condition when center of coordinate is center of figure too.
It is done by formula at each direction: min + (max - min)/2
*/
int move_to_center(obj_data *total_data, const double *extrems) {
    ...
    for (unsigned int i = 0; i < NUMBER_COORD_XYZ; ++i) {
        shift_values[i] = extrems[i * 2] +
                          (extrems[i * 2 + 1] - extrems[i * 2]) / 2;
    }
    return move_coordinate(&total_data->coordMatrix, -shift_values[0], -shift_values[1], -shift_values[2]);
}

// There is support function to resize object to field [-1;1] by each coordinates
...
It is created matrix of unique combinations of two vertex numbers (first is min of them, second - is max of them);
Circle of all polygons: each vertex with neighbour and last with first
*/
