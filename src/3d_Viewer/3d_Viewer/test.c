...
START_TEST(s21_move_xyz_m10_10) {
  ...
  for (unsigned int type = 0; type < 3; ++type) {
    for (double diff = -10; diff <= 10; diff += 5) {
      ...
      is_ok = init_coord_matrix(&test_result, total_data.number_vertex,
                                NUMBER_COLS);
      if (!is_ok) {
...
START_TEST(s21_edge_number) {
  ...
  if (is_ok) {
    int edge_number =
        get_edges_number(total_data.polygons, total_data.number_polygons);
...
START_TEST(s21_edge_number_big_3d) {
    ...
    int edge_number = get_edges_number(total_data.polygons,
total_data.number_polygons); ck_assert_int_eq(is_ok, OK);
