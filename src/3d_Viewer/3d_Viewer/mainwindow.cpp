...
MainWindow::MainWindow(QWidget *parent)
  ...
  if (ui->viewer_gl_widget->edge_color ==
      ui->viewer_gl_widget->background_color) {
    set_start_information();
  }

  connect(ui->button_move, SIGNAL(clicked()), this, SLOT(do_move()));
  connect(ui->button_scale_bigger, SIGNAL(clicked()), this,
          SLOT(do_scale_bigger()));
  connect(ui->button_scale_smaller, SIGNAL(clicked()), this,
          SLOT(do_scale_smaller()));
  connect(ui->button_rotate, SIGNAL(clicked()), this, SLOT(do_rotate()));
  connect(ui->button_edge_color, SIGNAL(clicked()), this,
          SLOT(set_color_edge()));
  connect(ui->button_vertex_color, SIGNAL(clicked()), this,
          SLOT(set_color_vertex()));
  connect(ui->button_background_color, SIGNAL(clicked()), this,
          SLOT(set_color_background()));
  connect(ui->button_do_extra_figure, SIGNAL(clicked()), this,
          SLOT(do_extra_figure_changes()));
  connect(ui->button_reset_extra_figure, SIGNAL(clicked()), this,
          SLOT(do_reset_figure_changes()));
  connect(ui->button_do_extra_system, SIGNAL(clicked()), this,
          SLOT(do_extra_system_changes()));
  connect(ui->button_reset_extra_system, SIGNAL(clicked()), this,
          SLOT(do_reset_system_changes()));
}
...
void MainWindow::save_settings() {
  ...
  settings.setValue("projection_type",
                    ui->viewer_gl_widget->is_projection_ortho);
  settings.endGroup();
...
void MainWindow::load_settings() {
  ...
  settings.beginGroup("Common");
  ui->viewer_gl_widget->background_color =
      settings.value("background_color").toString();
  ui->viewer_gl_widget->is_projection_ortho =
      settings.value("projection_type").toBool();
  settings.endGroup();
  ...
  ui->viewer_gl_widget->vert_type =
      (enum vertex_type)settings.value("vertex_type").toInt();
  ui->viewer_gl_widget->vertex_color =
      settings.value("vertex_color").toString();
  ui->viewer_gl_widget->vertex_depth =
      settings.value("vertex_depth").toDouble();
...
void MainWindow::on_button_pick_file_clicked() {
  QString filename_outside = QFileDialog::getOpenFileName(
      this, "Choooose", "./models/", "Object files (*.obj)");
  ...
  if (start(file_name_cstr, &ui->viewer_gl_widget->total_data) == 1) {
    ...
    ui->label_vertex_num_output->setText(
        QString::number(ui->viewer_gl_widget->total_data.number_vertex));
    ui->label_edge_num_output->setText(
        QString::number(ui->viewer_gl_widget->total_data.number_polygons));
    set_start_affin();
...
void MainWindow::do_move() {
  if (ui->viewer_gl_widget->is_file_loaded) {
    ...
    move_coordinate(&ui->viewer_gl_widget->total_data.coordMatrix, x_move_in,
                    y_move_in, z_move_in);
    ui->viewer_gl_widget->update();
  }
}
...
void MainWindow::do_scale_bigger() {
  if (ui->viewer_gl_widget->is_file_loaded) {
    ...
    if (scale_bigger != 0) {
      scale_coordinate(&ui->viewer_gl_widget->total_data.coordMatrix,
                       scale_bigger);
      ui->viewer_gl_widget->update();
      ...
      QMessageBox::warning(
          this, tr("Scale Error"),
          tr("Не масштабируй так, ведь бедную фигуру поглотят черные дыыыыры"));
      ui->line_scale_bigger->setText("");
      update();
    }
  }
...
void MainWindow::do_scale_smaller() {
  if (ui->viewer_gl_widget->is_file_loaded) {
    ...
    if (coefficient_from_user != 0) {
      ...
      scale_bigger = 0;  // чтобы одновременно не было коеффициента и в
                         // увеличителе и в уменьшителе
      ui->line_scale_bigger->setText("");
    } else {
      QMessageBox::warning(
          this, tr("Scale Error"),
...
void MainWindow::set_color_edge() {
  QColor edge_color = QColorDialog::getColor(Qt::white, this,
                                             "Выберите цвет отображения ребер");
  if (edge_color.isValid() &&
      edge_color != ui->viewer_gl_widget->background_color) {
...
void MainWindow::set_color_vertex() {
  QColor vertex_color = QColorDialog::getColor(
      Qt::white, this, "Выберите цвет отображения вершин");
  if (vertex_color.isValid() &&
      vertex_color != ui->viewer_gl_widget->background_color) {
    ui->viewer_gl_widget->vertex_color = vertex_color;
  }
}

void MainWindow::set_color_background() {
  QColor background_color = QColorDialog::getColor(
      Qt::white, this, "Выберите цвет отображения вершин");
  if (background_color.isValid() &&
      background_color != ui->viewer_gl_widget->edge_color) {
